	...
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4 && ajax.status == 200){
			...
			data.forEach(function(item){
				...
				listaElemento.appendChild(descricao);
				listaElemento.appendChild(valor);
		...
		function pegarInfo(el,tipo){
			if(tipo=="editar"){
				...
				var descricao = root.childNodes[1].innerHTML; 
				var valor = root.childNodes[2].innerHTML; 
		...
		function criarTipoDeIngresso(){
			var ajax = new XMLHttpRequest();
			var data = {
			descricao: document.getElementById('inserirDescricao').value,
			valor: parseFloat(document.getElementById('inserirValor').value),
		...
		function alterarTipoDeIngresso(el){
			var ajax = new XMLHttpRequest();
			var data = {
		...
		function deletar(id){
			...
			ajax.open("DELETE", URI+'tipoingresso'+"/"+id, true);
			ajax.send();
