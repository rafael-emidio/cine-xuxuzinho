	...
	ajax.onreadystatechange = function() {
		if(ajax.readyState == 4 && ajax.status == 200){
			...
			data.forEach(function(item){
				...
				listaElemento.appendChild(usuario);
				listaElemento.appendChild(senha);
		...
		function pegarInfo(el,tipo){
			if(tipo=="editar"){
				...
				var usuario = root.childNodes[1].innerHTML; 
				var senha = root.childNodes[2].innerHTML;
		...
		function criarUsuario(){
			var ajax = new XMLHttpRequest();
			var data = {
			usuario: document.getElementById('inserirUsuario').value,
			senha: document.getElementById('inserirSenha').value,
		...
		function deletar(id){
			...
			ajax.open("DELETE", URI+'usuarios'+"/"+id, true);
			ajax.send();
